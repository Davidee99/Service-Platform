...
public class LoginServiceImpl implements LoginService {
	...
	/**
	 * Metodo per fare il login tramite email e password che restituisce un response con
	 * all'interno:
	 * 
	...
	public ResponseEntity<?> login(AuthRequest authRequest) {
		...
		if (authRequest.getPassword() == null || authRequest.getEmail() == null) {
			return new ResponseEntity<LoginResponse>(response, HttpStatus.BAD_REQUEST); // 400
		}
		...
		try {
			...
			if (authentication.isAuthenticated()) {
				...
				if (loginInfo.getEmployee() == null) {
					...
					// Settiamo il Token tramite il metodo generateToken che utilizza l'email di loginInfo
		...
		return new ResponseEntity<LoginResponse>(response, HttpStatus.INTERNAL_SERVER_ERROR); // 500
	}
